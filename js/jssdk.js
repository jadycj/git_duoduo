...
function taobaoTaobaokeWidgetItemsConvert(parame){
    ...
    var fields='price,commission,commission_rate,promotion_price'; //可选字段num_iid,title,nick,pic_url,price,click_url,commission,commission_rate,commission_num,commission_volume,shop_click_url,seller_credit_score,item_location,volume
	...
	if(resp.error_response){
    	...
    	if(resp.total_results==0){
        	...
        	if(parame['onlyComm']==1){//只获取佣金即可
				...
				ddShowCommission(commission,promotion_price);
				return true;
        	}
    		else{
            	if(parame['ju_fxje']>0){
			...
			for(var k in a){
				commArr[a[k]['num_iid']]=a[k]['commission']
			}
		}

		if(ddFxje>=0){
			ddShowFxje(ddFxje,promotion_price);
		}
		else{
