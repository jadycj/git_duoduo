...
class tao_coupon extends plugin{
	function __construct($duoduo){
		parent::__construct($duoduo);
	}
	
	function get($shop_title,$goods_price){
		$str='';
		if($this->plugin['status']==1){
...
if($plugin_set[MY_PLUGIN_CODE]==1){
	$tao_coupon=new tao_coupon($duoduo);
	$tao_coupon_str=$tao_coupon->get($shop['title'],$goods['price']);
}
?>