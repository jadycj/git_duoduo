...
<table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
  ...
  <tr>
    ...
    <td>&nbsp;<input type="text" id="endtime" name="endtime" value="<?=$row['endtime']?>"/> <?php if($row['endtime']!='' && $endtime<date('Y-m-d H:i:s',TIME)){?>插件已过期<?php }?></td>
  </tr>
  <tr>
    <td align="right">模块/行为：</td>
    <td>&nbsp;<input type="text" name="mod" id="mod" style="width:90px" value="<?=$row['mod']?>" />/ <input type="text" id="act" name="act" style="width:90px" value="<?=$row['act']?>" /> 用于插件特别设置</td>
  </tr>
  <tr>
    <td align="right">导航标记：</td>
    <td>&nbsp;<input type="text" name="tag" id="tag" style="width:90px" value="<?=$row['tag']?>" /> 用于前台导航自动生成</td>
  </tr>
  
  <tr>
    <td align="right">适用版本：</td>
    <td>&nbsp;<input type="text" name="banben" id="banben" style="width:90px" value="<?=$row['banben']?>" /> 我当前的版本<?=BANBEN?></td>
  </tr>
  <?php if($row['search_open']==1){?>
  <tr>
    <td align="right">是否有前台搜索：</td>
    <td>&nbsp;<?=html_radio($shifou_arr,$row['search_open'],'search_open')?> 该插件是否需要前台搜索</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>&nbsp;<input type="text" name="search_width" id="search_width" value="<?=$row['search_width']?>" style="width:60px" /> 单位：像素（px） </td>
  </tr>
  <tr>
