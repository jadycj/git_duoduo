...
if($_GET['first']==1){
	...
	$total = $duoduo->count('tixian as m,user as b', $where.' and m.uid=b.id '.$type_where.$tool_where);
	$row = $duoduo->select_all('tixian as m,user as b', 'm.*,b.ddusername', $where . ' and m.uid=b.id '.$type_where.$tool_where.' order by m.id desc limit ' . $frmnum . ',' . $pagesize);
	$sum=(float)$duoduo->sum('tixian as m,user as b','m.money',$where.' and m.uid=b.id and m.status=0 '.$type_where.$tool_where.' order by m.id desc limit ' . $frmnum . ',' . $pagesize);
}
