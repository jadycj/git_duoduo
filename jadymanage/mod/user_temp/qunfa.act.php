...
if(isset($_GET['sub']) && $_GET['sub']!=''){
	...
	if($do=='sms'){
		...
		foreach($users as $row){
			...
			$re=$ddopen->sms_send($row['mobile'],$temp,999);
			//$re=array('s'=>1);
			if($re['s']==0){
				$data=array('msg'=>$re['r']);
				$duoduo->update($data,'id='.$row['id']);
				//jump(u(MOD,'list'),$re['r']);
			}
			else{
	...
	if(empty($ids)){
		...
		putInfo('<b style="color:red">已发送会员【'.$pagesize*$page.'】。。。</b><br/><img src="../images/wait2.gif" /><br/><a href="'.$url.'">如果浏览器没有跳转，请点击这里</a>',$url);
	}
	else{
		jump(-1,'发送完毕');
	}
