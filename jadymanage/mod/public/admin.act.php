...
switch (MOD) {
	...
	case 'mall_comment':
	    $malls1[0]='全部';
        $malls2=$duoduo->select_2_field('mall','id,title');
		if(empty($malls2)){$malls2=array();}
        $malls=$malls1+$malls2;
	break;
	...
	case 'nav':
		...
		$nav_arr1=$duoduo->select_2_field('nav');
	...
	case 'tradelist':
	    $checked_status=array(0=>'未核对',1=>'待审核',2=>'有效',-1=>'退款');
	break;
	...
	case 'tuan_goods':
	    $malls=$duoduo->select_2_field('mall','id,title','cid="'.$webset['tuan']['mall_cid'].'" and  api_url is not null and api_url<>"" and api_rule<>"" and api_rule is not null order by sort desc');
        $cat=$duoduo->select_2_field('tuan_type');
	break;
