...
if($_POST['sub']!=''){
	...
	if($id==0){
		if($_POST['act']=='' && $_POST['mod']==''){
		    ...
		    $data=array('parent_id'=>0,'node'=>$_POST['node'],'mod'=>$_POST['mod'],'act'=>$_POST['act'],'title'=>$_POST['title'],'hide'=>$_POST['hide'],'listorder'=>$_POST['listorder']);
		...
		elseif($_POST['act']=='' || $_POST['mod']==''){
		    ...
		    $data=array('parent_id'=>$parent_menuid,'node'=>$_POST['node'],'mod'=>$_POST['mod'],'act'=>$_POST['act'],'title'=>$_POST['title'],'hide'=>$_POST['hide'],'sort'=>$_POST['sort']);
			//print_r($data);exit;
			$menuid=$duoduo->insert('menu',$data);
			$data=array('role_id'=>1,'menu_id'=>$menuid);
			$duoduo->insert('menu_access',$data);
