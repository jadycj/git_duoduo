...
switch($act){
	...
	case 'tuan':
		...
		elseif($mall['lm']==5){
            $url="http://track.weiyi.com/sr.aspx?m=".$mall['weiyiid']."&w=".$webset['weiyi']['wzbh']."&d=0000&u=0&t=".urlencode($mall['iframe_url']);
        }
		elseif($mall['lm']==6){
            $url=$mall['wujiumiaourl'].'&to='.urlencode($mall['iframe_url']);
        }
        elseif($mall['lm']==3){
	    ...
	    elseif($mall['lm']==1){
			$qu=strstr($mall['chaneturl'],'&url=');
	        $url=str_replace($qu,'&url='.urlencode($mall['iframe_url']),$mall['chaneturl']);
