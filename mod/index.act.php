...
function dd_index_tao_goods($duoduo,$webset,$dduser,$ddTaoapi,$tao_hot_page,$num){
	if($webset['taoapi']['goods_show']==0){
		...
		foreach($tao_goods1 as $k=>$row){
        	$tao_goods1[$k]['fxje']=jfb_data_type(fenduan($row['commission'],$webset['fxbl'],$dduser['level'],TBMONEYBL));
...
function dd_index_dingdan($duoduo,$webset,$dduser,$ddTaoapi,$num=10){
	$num_iids='';
	$dingdaning=$duoduo->select_all('tradelist as a,user as b','a.item_title,a.num_iid,a.fxje,a.jifenbao,a.pay_price as price,a.commission_rate,a.uid,b.ddusername','a.uid=b.id and a.fxje>0 order by a.id desc limit '.$num);
	if(!empty($dingdaning)){
...
function dd_tuan($duoduo,$tuan_start_num,$num){
	$tuan_goods=$duoduo->select_all('tuan_goods as a,mall as b','a.title,a.img,a.price,a.value,a.rebate,a.id,b.fan,b.title as mall_name','a.mall_id=b.id and a.edatetime>"'.TIME.'" and city="全国" order by a.sort desc,a.salt desc limit '.$tuan_start_num.','.$num);
	return $tuan_goods;
}
