...
//幻灯片
$slides=$duoduo->select_all('slides','img,url,title','hide=0 and cid=4 order by sort asc limit 0,10');

$shops=$duoduo->select_all('shop','id,sort,tao_top,sid,pic_path,nick,level,fanxianlv,shop_click_url,title','1=1 order by tao_top desc, sort desc limit 0,7');

foreach($shops as $i=>$row){
	$shops[$i]["logo"]=TAOLOGO.$shops[$i]["pic_path"];
	$shops[$i]['fanxianlv']=$shops[$i]['fanxianlv'];
	if($shops[$i]['level']==21){
