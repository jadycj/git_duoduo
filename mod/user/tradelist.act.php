...
if($do=='taobao'){
	...
	$dingdan=$duoduo->left_join('tradelist as a','baobei AS b ON a.id = b.trade_id','a.id, a.item_title, a.num_iid, a.pay_price,a.real_pay_fee,a.fxje,a.jifenbao, a.commission,a.trade_id,a.pay_time, b.id as baobei_id',"a.uid=".$dduser['id']." and a.checked=2 order by a.id desc limit $frmnum,$pagesize");
}
elseif($do=='lost'){
	if(isset($_GET['q'])){
		$q=$_GET['q'];
	    ...
	    $dingdan=$duoduo->select_all('tradelist','id,item_title,pay_price,real_pay_fee,fxje,jifenbao,pay_time,trade_id','uid=0'.$where.' order by id desc limit '.$frmnum.','.$pagesize);
	}
}
elseif($do=='paipai'){
