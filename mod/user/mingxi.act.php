...
if($do=='in'){
	...
	$mingxi=$duoduo->select_all('mingxi','shijian,money,jifenbao,jifen,source,addtime',"uid='".$dduser['id']."' and (jifen>0 or money>0 or jifenbao>0) order by addtime desc limit $page2,$pagesize");
}
elseif($do=='out'){
...
elseif($do=='tui'){
	...
	$mingxi=$duoduo->select_all('mingxi','*',"uid='".$dduser['id']."' and shijian in (13,15) order by id desc limit $page2,$pagesize");
}
elseif($do=='kou'){
	$total = $duoduo->count('mingxi',"uid='".$dduser['id']."' and shijian in (21)");	
	$mingxi=$duoduo->select_all('mingxi','*',"uid='".$dduser['id']."' and shijian in (21) order by id desc limit $page2,$pagesize");
}
?>