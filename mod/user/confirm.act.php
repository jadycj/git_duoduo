...
if ($do == 'tao') {
	if ($_POST['sub'] == '') {
		...
		$trade = $duoduo->select('tradelist', 'id,trade_id,item_title,fxje,jifenbao,pay_price,item_num', 'id="' . $id . '"');
		if (!$trade['id']) {
			jump(-1,$errorData[46]);
		}
		...
		$trade=$duoduo->select('tradelist','id,checked,fxje,jifenbao,jifen,trade_id,commission,pay_time','id="'.$id.'"');
		if($trade['checked']!=0){
		    jump(-1,$errorData[45]);
		}
