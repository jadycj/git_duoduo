...
if(($webset['sms']['open']==1 && $webset['sms']['need_yz']==1) && ($msgset[2]['sms_open']==1 || $msgset[3]['sms_open']==1) && $dduser['mobile_test']==0){
	jump(u('user','info',array('do'=>'mobile')),'请验证您的手机号码');
}

if($type==1){
	if($dduser['alipay']==''){
		jump(u('user','info',array('do'=>'caiwu')),'请设置您的支付宝账号信息');
	}
...
elseif($type==2){
	if($dduser['alipay']=='' && $dduser['tenpay']=='' && $dduser['bank_code']==''){
		jump(u('user','info',array('do'=>'caiwu')),'请设置您的财务账号信息');
	}
...
if($dduser['realname']==''){
	jump(u('user','info',array('do'=>'caiwu')),'请设置您的真实姓名');
}

if($type==1){
	$tipword='您申请的提现我们会打入您的支付宝账户，请仔细填写您的支付宝和对应姓名，以免出错！';
...
if(isset($_POST['sub'])){
	...
	$data=array('uid'=>$dduser['id'],'money'=>$money,'addtime'=>TIME,'ip'=>get_client_ip(),'realname'=>$realname,'remark'=>$remark,'mobile'=>$mobile,'status'=>0,'type'=>$type);
	
	if($type==1){
		if($user['tbtxstatus']==1){
			jump(-1,38);
		}
		...
		if($dduser['alipay']!=''){
			...
			$user_data[]=array('f'=>'alipay','e'=>'=','v'=>$data['code']);
		}
		if($data['code']==''){
			jump(-1,35);  //支付宝格式错误
		}
	...
	elseif($type==2){
		...
		elseif($tool==2){
			if($dduser['tenpay']!=''){
				...
				$user_data[]=array('f'=>'tenpay','e'=>'=','v'=>$data['code']);
			}
			if($data['code']==''){
				jump(-1,62);  //财付通格式错误
			}
		...
		elseif($tool==3){
			...
			elseif($_POST['bank_id']>0 && $_POST['bank_code']!=''){
				...
				$user_data[]=array('f'=>'bank_name','e'=>'=','v'=>$_POST['bank_name']);
				$user_data[]=array('f'=>'bank_code','e'=>'=','v'=>$_POST['bank_code']);
			}
			else{
				jump(-1,60);  //银行账号格式错误
			}
	...
	$user_data[]=array('f'=>$tx_field,'v'=>1);
	$user_data[]=array('f'=>$money_field,'e'=>'-','v'=>$money);
	$user_data[]=array('f'=>'lasttixian','e'=>'=','v'=>TIME);
	
	if($dduser['mobile']=='' && $mobile!=''){
		$user_data[]=array('f'=>'mobile','e'=>'=','v'=>$mobile);
	}
	...
	if($dduser['tenpay']!=''){$tenpay_dis='disabled="disabled"';}
	if($dduser['bank_code']!=''){$bank_code_dis='disabled="disabled"';}
