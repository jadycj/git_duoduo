...
class ddTaoapi extends Taoapi{
	...
	function items_detail_get($Tapparams,$url=''){
		...
		if($url!=''){
			...
			if(strpos($url,'ju.taobao.com')!==false){  //聚划算商品
				...
				if($goods['notaoke']==1){
					$goods['click_url']=$this->taobao_taobaoke_t9('http://detail.tmall.com/item.htm?id='.$goods['num_iid'],$Tapparams['outer_code']);  //不能采用t9转换，丢单
				}
				$ju_html=dd_get($url);
				preg_match('/<strong class="J_juPrices"><b>&yen;<\/b>(.*)<\/strong>/',$ju_html,$a);
				$ju_price=(float)$a[1];
				if($ju_price>0){
					$goods['price']=$ju_price;
	...
	function taobao_taobaoke_shops_relate_convert($Tapparams){
		$this->method='taobao.taobaoke.shops.relate.get';
		$this->fields='user_id';
		$this->seller_nick=$Tapparams['seller_nick'];
		$TaobaokeData = $this->Send('get',$this->format)->getArrayData();
		return $TaobaokeData;
	}
}
