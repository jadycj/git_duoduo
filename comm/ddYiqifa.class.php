...
class ddyiqifa extends yiqifa{
	...
	function product_search($parame,$total=0){  //搜索商品
		...
		foreach($row as $k=>$arr){
			...
			$row[$k]['title']=dd_replace($row[$k]['productName'],$this->nowords);
			$row[$k]['mall_name']=$row[$k]['merchantName'];
			$row[$k]['mall_url']=$row[$k]['merchantTopurl'];
			$row[$k]['pic_url']=$row[$k]['picurl'];
			$row[$k]['price']=$row[$k]['showPrice'];
			$row[$k]['market_price']=$row[$k]['showRefPrice'];

			unset($row[$k]['productName']);
			unset($row[$k]['merchantName']);
			unset($row[$k]['merchantTopurl']);
			unset($row[$k]['picurl']);
			unset($row[$k]['showPrice']);
			unset($row[$k]['showRefPrice']);
			
			unset($row[$k]['browseNodeKeyword']);
			unset($row[$k]['categoryName']);
			unset($row[$k]['discounturl']);
			unset($row[$k]['pinPai']);
			unset($row[$k]['refprice']);
			unset($row[$k]['searchKeyword']);
			unset($row[$k]['shortIntro']);
			unset($row[$k]['unionCode']);	
		}
