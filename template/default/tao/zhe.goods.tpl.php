...
<div class="item clearfix">
    ...
    <div class="t_a">
        <div class="img lazy-img-loader">            
            <a href="<?=$row['gourl']?>" class="redirect-tbk" target="_blank">
        ...
        <div class="title">
            <a href="<?=$row['gourl']?>" class="blue_link redirect-tbk" target="_blank"><?=$row["title"]?></a>
        </div>
        <div style="padding-left:5px; padding-bottom:5px">结束时间：<?=tranTime($row["coupon_end_time"])?></div>
        <div class="rebate" >
			<span title="<?=TBFLTIP?>">折后返<em><?=$row["coupon_fxje"]?></em><?=TBMONEY?>，<?=$row['volume']?>人已购买</span>
		</div>
