	...
	$.ajax({
		...
		success: function(data){
			if(data.s==0){
				...
				if(data.id==18){
					var tao;
				    helpWindows('商品加载失败或者该商品已下架。<br/>查看商品链接：<a target="_blank" href="'+url+'">'+url+'</a>','<?=WEBNICK?>小助手');
				}
			}
			else if(data.s==1){
...
function ddShowCommission(comm){
	if(comm>=0){
		commission=comm;
		$('#subShare').attr('disabled','');
	} 
}
	...
	$('#shaiForm').submit(function(){
		...
		if(typeof image=='undefined' || image==''){
			...
			$.ajax({
		        ...
		        type: "POST",
		        data:{title:title,tao_id:tao_id,price:price,commission:commission,image:image,keywords:keywords,comment:comment,cid:cid,click_url:clickUrl,nick:nick,trade_id:trade_id},
		        dataType: "json",
		        success: function(data){
			        if(data.s==0){
						$('#subShare').attr('disabled','');
